...
import { Btn, Badge, StatCard, InsightCard, SectionTitle, fmtINR, ChartTooltip, C } from '../../components/ui'
  ...
  return (
    <Layout title={vehicle.name} subtitle={`${vehicle.regNumber} · ${vehicle.fuelType}`}
      actions={
        <div style={{ display:'flex', gap:'8px' }}>
          ...
          <Btn size="sm" onClick={()=>router.push(`/fuel/new?vid=${id}`)}>⛽ Log Fuel</Btn>
          <Btn size="sm" variant="secondary" onClick={()=>router.push(`/odometer/new?vid=${id}`)}>🛞 Update Odo</Btn>
    ...
    >
      <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(150px, 1fr))', gap:'12px', marginBottom:'20px' }}>
        ...
        <StatCard label="Distance" value={analytics.totalKm ? `${analytics.totalKm.toLocaleString('en-IN')} km` : '—'} icon="🗺️" accent="var(--accent3)" delay={120} />
        <StatCard label="Cost / km" value={analytics.costPerKm ? `${C}${analytics.costPerKm}` : '—'} icon="📏" accent="var(--accent2)" delay={180} />
        <StatCard label="Avg Mileage" value={analytics.avgMileage ? `${analytics.avgMileage} km/L` : '—'} icon="⚡" accent="#a78bfa" delay={240} />
        <StatCard label="Next Service" value={nextServiceKm!==null?(nextServiceKm>0?`${nextServiceKm.toLocaleString('en-IN')} km`:'Overdue'):'—'} icon="🔧" accent={nextServiceKm!==null&&nextServiceKm<=0?'var(--danger)':nextServiceKm!==null&&nextServiceKm<500?'var(--warning)':'var(--success)'} delay={300} />
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px', marginBottom:'14px' }}>
        ...
        <Card title="Mileage Trend">
          {!analytics.mileageTrend?.length ? <NoDat text="Log fuel to see mileage trend" /> :
            <ResponsiveContainer width="100%" height={160}>
            ...
            return (
              <div key={f.id} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'9px 0', borderBottom:'1px solid var(--border)' }}>
                ...
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:'13px', fontWeight:700 }}>{fmtINR(f.cost)}</div>
                  {f.mileage && <div style={{ fontSize:'11px', color:'var(--accent3)' }}>{f.mileage} km/L</div>}
                </div>
              </div>
            )
          })}
